...
//get influencers
int_t *get_influencers(varNode *Node, int_t N, int_t **Graph, int L, int ntwk) {
	...
	THRESHOLD = (dbl_t)bigCompNodes(Node, Graph, N, queue)/N;
	cnt = 0;
...
int main(int argc, char *argv[]){
	int_t i, N, G, **Graph, *listInfluencers;
	int L, ntwk;
	const char *network; 
	...
	L = atoi(argv[2]);
    ntwk = atoi(argv[1]);
	sprintf(fname_infl,"output.csv",ntwk,L);
	...
	//GET INFLUENCERS
	listInfluencers = get_influencers(Node, N, Graph, L, ntwk);
	
	fprintf(stdout, "\t\t\t\t  ### NETWORK %d DONE ###\n\n", ntwk);	
